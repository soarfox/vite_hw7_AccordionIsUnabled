...
<template>
  <div class="container my-5">
    ...
    <div class="row" v-if="!this.$route.params.id">
      ...
      <!-- 設定右側有col-10 -->
      <div class="col-10">
        <!-- 使用Bootstrap 5的格線系統(row)來製作, 以row為起手式語法, 加上row-cols-1(要分成幾欄), row-cols-md-3代表一欄有3張卡片, 且為了預留一點空白空間, 避免該區塊距離上下方太靠近, 因此使用my, 代表上下方的margin(元素與元素之間的邊界距離), 輸入5代表一定距離; 因為兩張卡片的距離貼得太靠近, 因此使用gutters(g-4)來增加垂直與水平的距離, 可參考: https://www.casper.tw/development/2020/10/10/bootstrap-5-grid-whats-new/, 因此在此設定g-4(將垂直與水平距離都設定為4這個尺寸)-->
...
export default {
  // 在路由表(router/index.js)內定義一個參數category, 在此要使用props進行接收
  props: ['category'],
  data () {
    return {
  ...
  mounted () {
    if (this.$route.params.category === undefined) {
      ...
      console.log('無分類資料, 顯示全部商品')
      this.getPastaAll()
    } else {
      console.log('顯示分類商品')
      console.log('this.$route.params.category=', this.$route.params.category)
      this.getPastaCategoryProducts(this.$route.params.category)
    }
  }
}
</script>
