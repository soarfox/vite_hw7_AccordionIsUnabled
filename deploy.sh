...
# 當Vue程式碼有任何更動後, 請記得先在VScode終端機內使用指令npm run build後, 再將變動的內容寫上註解並commit出去, 最後才使用(在Windows作業系統下)請打開資料夾視窗, 請對本專案資料夾按下右鍵, 選擇以"git Bash here"方式打開bash視窗, 輸入這個指令sh deploy.sh
